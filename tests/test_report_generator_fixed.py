...
class TestReportGenerator(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.mock_performance_viz_class = self.performance_viz_patcher.start()
        self.mock_performance_viz = MagicMock()
        self.mock_performance_viz_class.return_value = self.mock_performance_viz

        self.portfolio_analytics_patcher = patch(
            "src.api.visualization.portfolio_analytics.PortfolioAnalytics"
        )
        self.mock_portfolio_analytics_class = self.portfolio_analytics_patcher.start()
        self.mock_portfolio_analytics = MagicMock()
        self.mock_portfolio_analytics_class.return_value = self.mock_portfolio_analytics

        self.risk_analysis_patcher = patch(
            "src.api.visualization.risk_analysis.RiskAnalysis"
        )
        self.mock_risk_analysis_class = self.risk_analysis_patcher.start()
        self.mock_risk_analysis = MagicMock()
        self.mock_risk_analysis_class.return_value = self.mock_risk_analysis
    ...
    def test_generate_strategy_performance_report(self):
        ...
        self.mock_performance_viz.create_performance_dashboard.return_value = (
            mock_dashboard
        )

        # Mock portfolio dashboard
        mock_portfolio = {"portfolio_composition": {"data": "test"}}
        self.mock_portfolio_analytics.create_portfolio_dashboard.return_value = (
            mock_portfolio
        )
    ...
    def test_analyze_arbitrage_opportunities(self):
        ...
        # Create sample data
        data = []
        timestamps = pd.date_range(start="2023-01-01", periods=5, freq="1h")
    ...
    def test_calculate_arbitrage_metrics(self):
        ...
        # Create sample data with a 1-day range
        data = pd.DataFrame({
        ...
        # Mock the total_seconds method to return a fixed value for testing
        with patch.object(
            timedelta, "total_seconds", return_value=86400
        ):  # 1 day in seconds
            # Calculate metrics
            ...
            self.assertEqual(metrics["opportunity_frequency"], 3.0)
    ...
    @patch("src.reporting.report_generator.report_generator")
    def test_module_functions(self, mock_generator):
